...
!(function (t, i) {
  ...
  (t.Control.BigImage = t.Control.extend({
    ...
    onAdd: function (t) {
      ...
      return (
        -1 != e.indexOf("&") && n.push(this.options._unicodeClass),
    ...
    _click: function (t) {
      ...
      this._print().then(() => {
        ...
        // Show a simple success message (can be improved with custom UI)
    ...
    _getLayers: function (i) {
      ...
      e._map.eachLayer(function (i) {
        n.push(
          new Promise((n) => {
            try {
              i instanceof t.Marker
                ? e._getMarkerLayer(i, n)
                : i instanceof t.TileLayer
                  ? e._getTileLayer(i, n)
                  : i instanceof t.Circle
                    ? (e.circles[i._leaflet_id] || (e.circles[i._leaflet_id] = i),
                      n())
                    : i instanceof t.Path
                      ? e._getPathLayer(i, n)
                      : n();
    ...
    _getTileLayer: function (i, e) {
      ...
      for (let o = n.tileBounds.min.y; o <= n.tileBounds.max.y; o++)
        for (let s = n.tileBounds.min.x; s <= n.tileBounds.max.x; s++)
          n.tiles.push(new t.Point(s, o));
      let a = [];
      n.tiles.forEach((e) => {
        let o = e.clone();
    ...
    _loadTile: function (t, i, e, n) {
      ...
      (a.crossOrigin = "Anonymous"),
        (a.onload = function () {
          o.tilesImgs[e._leaflet_id][s] ||
            (o.tilesImgs[e._leaflet_id][s] = { img: a, x: i.x, y: i.y }),
            n();
        }),
    ...
    _getMarkerLayer: function (i, e) {
      ...
      if (
        ((o = o.subtract(new t.Point(n.bounds.min.x, n.bounds.min.y))),
          i.options.icon &&
          i.options.icon.options &&
          i.options.icon.options.iconAnchor &&
          ((o.x -= i.options.icon.options.iconAnchor[0]),
            (o.y -= i.options.icon.options.iconAnchor[1])),
          !n._pointPositionIsNotCorrect(o) && i._icon.src)
      ) {
        let s = new Image();
        (s.crossOrigin = "Anonymous"),
          (s.src = i._icon.src),
          (s.onload = function () {
            (n.markers[i._leaflet_id] = { img: s, x: o.x, y: o.y }), e();
          });
        return;
      }
      if (n._pointPositionIsNotCorrect(o) || !i._icon.innerHTML || i._icon.src)
        e();
      else {
        let a = new Text(i._icon.innerHTML);
        (n.markers[i._leaflet_id] = { html: a, x: o.x, y: o.y }), e();
      }
    },
    ...
    _print: async function() {
      let t = this;
      t._containerParams.classList.add("print-disabled");
      t._loader.style.display = "block";
      ...
      try {
          ...
          await new Promise(t._getLayers.bind(t));
          ...
          for (let [r, h] of Object.entries(t.markers)) {
              h.img ? t.ctx.drawImage(h.img, h.x, h.y) : t._drawText(h, h.x, h.y);
          }
          for (let [d, p] of Object.entries(t.circles)) t._drawCircle(p);
  
          // Convert to blob
          const blob = await new Promise(resolve => {
              t.canvas.toBlob(resolve, 'image/png');
          });
          ...
          if ('showSaveFilePicker' in window) {
              try {
                  const handle = await window.showSaveFilePicker({
                      suggestedName: `map_${new Date().toISOString().slice(0,10)}.png`,
                      types: [{
