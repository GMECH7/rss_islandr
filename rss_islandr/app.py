"""Start of the app: window, command line arguments and the internal modes (self-test, map process)."""
...
def start_ui() -> None:
    ...
    main_ui = MainAppUI(root)
    main_ui.create_ui()
    loading_frame.destroy()
    root.protocol("WM_DELETE_WINDOW", main_ui.on_closing)
    root.mainloop()


def run(argv: list[str] | None = None) -> int:
    """Entry point of the app (`poetry run islandr`, `python main.py` and the built executables)."""
    try:
